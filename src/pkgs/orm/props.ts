...
import {PropType, prop as tgprop, ModelOptions, getModelForClass, index} from "@typegoose/typegoose";
...
export function prop(typeopts?: TypeOpts, kind?: PropType, opts?: PropOpts): PropertyDecorator {
    ...
    return function (target, propertyKey) {
        ...
        let schemaInfo = schemaReflectInfos.get(target.constructor) || {url: "", opts: {}};
        if (!schemaInfo.fields) {
            schemaInfo.fields = new Map;
        }
        schemaInfo.fields.set(propertyKey, {typeopts: typeopts, kind, opts: opts});
